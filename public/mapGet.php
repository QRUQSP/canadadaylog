...
// Binary image data
//
function qruqsp_canadadaylog_mapGet(&$ciniki) {
    //
    // Check args
    //
...
//        'sections'=>array('required'=>'no', 'blank'=>'yes', 'type'=>'list', 'name'=>'Sections'), 
    ...
    $rc = qruqsp_canadadaylog_checkAccess($ciniki, $args['tnid'], 'qruqsp.canadadaylog.mapGet');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.canadadaylog.18', 'msg'=>'', 'err'=>$rc['err']));
    }
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.canadadaylog.26', 'msg'=>'', 'err'=>$rc['err']));
    }
    if( !is_dir($rc['cache_dir']) ) {
        mkdir($rc['cache_dir'], 0755, true);
    }
    $cache_file = $rc['cache_dir'] . '/canadadaymap.jpg';
    

    if( is_array($sections) && implode(',', $sections) == $cache_map_sections && file_exists($cache_file)) {
        $map = new Imagick($cache_file);
    } else {
/*        $map = imagecreatefrompng($ciniki['config']['qruqsp.core']['modules_dir'] . '/canadadaylog/maps/back_with_lines.png');
        if( isset($args['sections'][0]) && $args['sections'][0] != '' ) {
            foreach($args['sections'] as $s) {
                if( file_exists($ciniki['config']['qruqsp.core']['modules_dir'] . '/canadadaylog/maps/' . $s . '.png') ) {
                    $overlay = imagecreatefrompng($ciniki['config']['qruqsp.core']['modules_dir'] . '/canadadaylog/maps/' . $s . '.png');
...
//        $ciniki['session']['qruqsp.canadadaylog']['map_sections'] = $sections;
