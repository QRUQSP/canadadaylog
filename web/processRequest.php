...
// -------
//
function qruqsp_canadadaylog_web_processRequest(&$ciniki, $settings, $tnid, $args) {

    //
    // Check to make sure the module is enabled
    //
    if( !isset($ciniki['tenant']['modules']['qruqsp.canadadaylog']) ) {
        return array('stat'=>'404', 'err'=>array('code'=>'qruqsp.canadadaylog.5', 'msg'=>"I'm sorry, the page you requested does not exist."));
    }
    ...
    if( count($page['breadcrumbs']) == 0 ) {
        if( isset($settings['page-canadadaylog-name']) && $settings['page-canadadaylog-name'] != '' ) {
            $page['breadcrumbs'][] = array('name'=>$settings['page-canadadaylog-name'], 'url'=>$args['base_url']);
        } else {
    ...
    if( isset($args['uri_split'][0]) && $args['uri_split'][0] == 'map' ) {
        ...
        $rc = ciniki_core_dbQueryList($ciniki, $strsql, 'qruqsp.canadadaylog', 'sections', 'section');
        if( $rc['stat'] != 'ok' ) {
            return $rc;
        }
        ...
        $rc = ciniki_core_dbDetailsQuery($ciniki, 'qruqsp_canadadaylog_settings', 'tnid', $tnid, 'qruqsp.canadadaylog', 'settings', '');
        if( $rc['stat'] != 'ok' ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.canadadaylog.15', 'msg'=>'', 'err'=>$rc['err']));
        }
        ...
        if( $rc['stat'] != 'ok' ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.canadadaylog.27', 'msg'=>'', 'err'=>$rc['err']));
        }
        ...
        if( is_array($sections) && implode(',', $sections) == $cache_map_sections && file_exists($cache_file)) {
            ...
            $map = new Imagick($ciniki['config']['qruqsp.core']['modules_dir'] . '/canadadaylog/maps/back_with_lines.png');
          
            if( count($sections) > 0 ) {
                foreach($sections as $s) {
                    if( file_exists($ciniki['config']['qruqsp.core']['modules_dir'] . '/canadadaylog/maps/' . $s . '.png') ) {
                        $overlay = new Imagick($ciniki['config']['qruqsp.core']['modules_dir'] . '/canadadaylog/maps/' . $s . '.png');
    ...
    elseif( isset($args['uri_split'][0]) && $args['uri_split'][0] == 'sections' ) {
        ...
        $rc = qruqsp_canadadaylog_sectionsLoad($ciniki, $tnid);
        if( $rc['stat'] != 'ok' ) {
            return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.canadadaylog.22', 'msg'=>'', 'err'=>$rc['err']));
        }
