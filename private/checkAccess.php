...
// <rsp stat='ok' />
//
function qruqsp_canadadaylog_checkAccess(&$ciniki, $tnid, $method) {
    //
    ...
    $rc = ciniki_tenants_checkModuleAccess($ciniki, $tnid, 'qruqsp', 'canadadaylog');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    $modules = $rc['modules'];
    ...
    if( !isset($rc['ruleset']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.canadadaylog.1', 'msg'=>'No permissions granted'));
    }
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.canadadaylog.2', 'msg'=>'Access denied.'));
    }
    //
    ...
    // By default fail
    //
    return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.canadadaylog.3', 'msg'=>'Access denied'));
}
?>
